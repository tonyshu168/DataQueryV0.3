angular.module("dataQuery",["ng"]).controller("mainCtrl",["$scope","$rootScope","$http","$interval","$timeout",function($scope,$rootScope,$http,$interval,$timeout){
    ...
    $scope.showExportExcel = function(){  //导出到Excel
        $("#exportExcel").show(500)
    }
    ...
    $scope.refresh = function(){
        ...
        if($scope.uname !="apy"){
            ...
            //获取安装量
            $http.get("data/getProducts_installCount.php?name="+$rootScope.uname).success(function(data){
            ...
            $http.get("data/getAllProducts_pager.php?pno=1").success(function(data){
                ...
                //获取总安装量
                $http.get("data/getAllProducts_installCount.php").success(function(data){
                    $rootScope.total = $scope.getInstallTotal(data);
                });
            });
        }
    ...
    $scope.changeDate = function(millonS){
        ...
        if(/^\d+$/g.test(millonS)){
            newDate = newDate.toLocaleDateString().replace(/\/|[\u4e00-\u9fa5]/g,"-");
        return newDate.replace(/-$/g,"");
        }
        else{
            newDate = millonS.replace(/\/|[\u4e00-\u9fa5]/g,"-");   //去掉年月日
            newDate = newDate.replace(/-$/g,"");
        var newDateChange = newDate.replace(/\d/g,"*");                           //方便查找年份
        var yearIndex = newDateChange.lastIndexOf("****");                        //找到年份的位置
        yearIndex > 1 && (newDate = newDate.substr(yearIndex,4) + newDate.substring(0,yearIndex-1));              //将 "12-22-2016" => "2016-12-22"
        newDateChange = null;
        yearIndex = null;
        return newDate;
        }
    ...
    $scope.changeEffeDate = function(data){
        for(var i = 0; i < data.length; i++){
            ...
            data[i].effectiveDate = date.toLocaleDateString().replace(/\/|[\u4e00-\u9fa5]/g,"-").replace(/-$/,"");
        }
        return data;
    }
    ...
    $scope.checkDate = function(date){
        ...
        if(dateReg.test(date) && dateReg02.test(date.substr(date.length-2)) || dateReg03.test(date)){
            if(dateReg03.test(date)){date = (date.replace(/(年|月)/g,"-").substr(0,10));}   //取出 “年月日"
    ...
    $scope.saveEditData = function($index){
        if($scope.editable == false){
            ...
            var installCount = $scope.products[$index].installCount;

            if(installCount && $scope.checkDate($scope.products[$index].effectiveDate) && checkAdId){
    ...
    const PAGEQUANTITY = 18;   //用于设置页签的个ovtn
    //创建分页标签
    ...
    $scope.pagerClick = function(){
        ...
        $("ul.pager_custom >li > a").on("click",function(e){
            ...
            if($("#self_company").css("display") != "none"){         //如果 #self_company display = "block",就到#mainContent 下找 分页按钮
            ...
            else{                                                       //否则到#other_user 下找 分页按钮
                var lis = $("#other_user > ul.pager_custom > li").length;    //获取所有li的数量
                var currentIndex = $("#other_user > ul.pager_custom > li").index(this.parentNode);
            }
            ...
            if(/\d/.test(target.innerHTML)){
                ...
                changeAllValue(currentIndex,PAGEQUANTITY+2)
            }
        ...
        function checkEllipsis(drection, lis){
            ...
            //倒数第二个页签的值等于page_count-2时，并且为当前页
        ...
        function changeAllValue(index, lis){
            if($("#self_company").css("display") != "none"){
            var currentValue = parseInt($('#mainContent >ul.pager_custom > li').eq(index).find("a").html()); //获取当前值
            var All_a = $("#mainContent >ul.pager_custom > li > a");   //找到所有ul > li >a
            }
            else{
                var currentValue = parseInt($('#other_user >ul.pager_custom > li').eq(index).find("a").html()); //获取当前值
                var All_a = $("#other_user >ul.pager_custom > li > a");   //找到所有ul > li >a
            }
            /************/
            if(currentValue < page_count && currentValue + COUNT > lis-2 && page_count > PAGEQUANTITY){    //当前页小于总页数是时，并且当前a的值如加上COUNT的值大于所有a值为数字的数量, 并且总页面数要大于PAGEQUANTITY
                $(All_a[1]).parent().next()[0].tagName != "SPAN" && $(All_a[1]).parent().after('<span>...</span>');      //在标签1次元素后添加 "..."
                console.log(page_count)
                if(currentValue == page_count-1) {  //当前页签为倒数第二页时
                    ...
                    console.log(All_a[lis-2].innerHTML)
                    if (All_a[lis-2].innerHTML < page_count ) {     //最后一个页签的值小于page_count
                    //最后一个页签与第二页签的值全部减一
                    for (var i = lis - 2; i >= 2; i--) {
                        All_a[i].innerHTML = parseInt(All_a[i].innerHTML) + 1;
                    }
                ...
                for(var i = lis- 2, j = 0; i >= 2; i--, j++){
                    ...
                    if(All_a[i].innerHTML == currentValue ){$(All_a[i]).parent().addClass("active").siblings(".active").removeClass("active")}
                        ...
                        $(All_a[currentValue-2]).parent().addClass("active").siblings(".active").removeClass("active");
            ...
            else if(currentValue == page_count){   //如果是最后一页或倒数第二页(2017-1-3)
                if($("#self_company").css("display") != "none"){
                    $('#mainContent > ul.pager_custom > li').eq(index).addClass("active").siblings(".active").removeClass("active");
                }
                else{
                    $('#other_user > ul.pager_custom > li').eq(index).addClass("active").siblings(".active").removeClass("active");
                }
            }
            else{
                if($("#self_company").css("display") != "none"){
                ...
                //总页数大于 页签个数时，才调整页签值
                var lis = $("#self_company").css("display") != "none" ? $("#mainContent > ul.pager_custom > li").length : $("#other_user > ul.pager_custom > li").length;
                if(page_count > lis-2){           //新加的判断(2017-1-3)
                    for(var i = 2; i < lis-1; i++){     //19
                        All_a[i].innerHTML = i;
                        $(All_a[1]).parent().next()[0].tagName == "SPAN" ? $(All_a[1]).parent().next().remove() : "";
                    }
        ...
        */
        ...
        if($scope.productName && $scope.channelNum && $scope.company && $scope.compAbbreviation && $scope.cooperateType
            ...
            //if(($scope.effectiveDate.getTime()+1000*3600*24*365) <= $scope.endDate.getTime()) { //用于检查用户输入格式是否正确
                var param = "adPlatform=" + $s.adPlatform + "&adID=" + $s.adID + "&proName=" + encodeURIComponent($rootScope.productName) + "&channelName=" +
                    $s.channelNum + "&company=" + encodeURIComponent($rootScope.company) + "&compabbr=" + $s.compAbbreviation + "&coopType=" + $s.cooperateType
            ...
            /*
            console.log($rootScope.productName,$rootScope.channelNum,$rootScope.company,$scope.compAbbreviation,$scope.cooperateType
                , priceMsg, installMsg,$s.effectiveDate, $s.adPlatform, $s.adID);
                */
            $scope.isVisi = "visible";
...
}]).controller("exportExcel",["$scope","$http","$rootScope",function($scope,$http,$rootScope){
    $scope.dateAlert = $scope.pageAlert = "hidden"              //用于控制日期与页数选择的不正确
    var selectDate = $(".selectDate"),
        selectPage = $(".selectPage"),
        checkbox = $("#exportExcel .currentPage > input[type='checkbox']"),
        dateCount = 0,             //计数器，用于让其它选择为不可用(只要执行一次,防止重复执行)
        pageCount = 0;

    $scope.close = function(){
        $("#exportExcel").hide(500)
        clear()
    }

    //选择当前页
    $scope.selectCurrentPage = function(){
        checkbox.attr("disabled") && checkbox.removeProp("disabled")
        if(checkbox[0].checked){
            selectDate.addClass("cancel").find("input").prop("disabled",true)
            selectPage.addClass("cancel").find("input").prop("disabled",true)
        }
        else{
            selectDate.removeClass("cancel").find("input").removeProp("disabled")
            selectPage.removeClass("cancel").find("input").removeProp("disabled")
        }
    }

    //点击日期时，其它选择项变为不可选
    $scope.selectDate = function(){
        dateCount < 1 && disabled("selectDate")    //让其它选择项变为不可选,并执行一次
        dateCount ++
        $scope.dateAlert == "visible" && ($scope.dateAlert = "hidden")
    }

    //点击页数时，其它顶变为不可选
    $scope.selectPage = function(){
        pageCount < 1 && disabled("selectPage")
        pageCount ++
        $scope.pageAlert == "visible" && ($scope.pageAlert = "hidden")
    }


    //导出数据到Excel(是选择日期，还是页数，还是当前页)
    $scope.submit = function(){
        selectDate = $("div.selectDate"),
            selectPage = $("div.selectPage"),
            checkbox = $("#exportExcel .currentPage > input[type='checkbox']");

        if(!checkbox.attr("disabled")){
            var currentPage = $("ul.pager_custom > li.active").find("a").html();
            $http.get("data/phpExcel/project/export.php?currentPage=" + currentPage).success(function(data){
                data = data ? data : "http://ad.appholly.com/data/phpExcel/project/export.xls"               //服务器传输入错就误
                console.log(data)
                open(location.href = data)           //下载Excel文件
            })
            $scope.close()
            return
        }
        if(!selectDate.hasClass("cancel")){
            $http.get("data/phpExcel/project/export.php?effectiveDate=" + $scope.effectiveDate + "&endDate=" + $scope.endDate).success(function(data){
                data = data ? data : "http://ad.appholly.com/data/phpExcel/project/export.xls"               //服务器传输入错就误
                open(location.href = data)           //下载Excel文件
            })
            $scope.close()
        ...
        if(!selectPage.hasClass("cancel")){
            $http.get("data/phpExcel/project/export.php?startPage=" + $scope.startPage + "&finishPage=" + $scope.finishPage).success(function(data){
                data = data ? data : "http://ad.appholly.com/data/phpExcel/project/export.xls"               //服务器传输入错就误
                $scope.close()
                open(location.href = data)           //下载Excel文件
            })
    ...
    $scope.compareDate = function(){
        if(!$scope.effectiveDate || !$scope.endDate) return
        $scope.effectiveDate = $scope.effectiveDate.getTime(),          //将日期转换成时间戳
        $scope.endDate = $scope.endDate.getTime();

        if(Math.abs($scope.endDate - $scope.effectiveDate)/(1000*3600*24*365) > 1){   //如果总时间数大于一年
            $scope.dateAlert = "visible"
        }
    }

    //检查页数输入是否在正确范围内
    $scope.comparePage = function(){
        if(!$scope.startPage || !$scope.finishPage) return

        if(Math.abs($scope.finishPage - $scope.startPage) > 18){$scope.pageAlert = "visible"}
    }

    //让对应的选项不可用:如选择时间则让 按页导出 与 选择当前页 不可选
    disabled = function(currentTag){
        if(currentTag != "selectDate"){
            selectDate.addClass("cancel").find("input").prop("disabled",true)
        }
        if(currentTag != "selectPage"){
            selectPage.addClass("cancel").find("input").prop("disabled",true)
        }
        if(currentTag != "currentPage"){
            checkbox.closest("div").addClass("cancel")
    ...
    clear = function(){
        selectDate.removeClass("cancel").find("input").removeProp("disabled")
        selectPage.removeClass("cancel").find("input").removeProp("disabled")
        checkbox.closest("div").removeClass("cancel")
        checkbox.removeProp("disabled")
        dateCount = pageCount = null
        checkbox[0].checked = false

        $scope.effectiveDate = $scope.endDate = null
        $scope.startPage = $scope.finishPage = null
    }
}]);